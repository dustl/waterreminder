...
1. **定位** — 启动时通过 ipinfo.io 获取 IP 地理位置 → 经纬度

1. **天气** — 调用 Open-Meteo 免费天气 API，缓存到 UserDefaults，每小时刷新

1. **图标** — 菜单栏图标根据天气切换对应 SF Symbol

1. **计时** — 按设定间隔倒计时（遵守工作时段），到点时弹出自定义面板

1. **面板** — 无边框 NSPanel + NSVisualEffectView（popover 材质），从顶部 ease-out 滑入

1. **浮层** — 喝水后小面板显示鼓励语，2 秒自动收起

1. **状态** — 同样滑动画风，显示距上次提醒时间、饮水量、间隔、工作时段

1. **记录** — 饮水历史以 `[日期: 毫升]` 字典存入 UserDefaults

1. **午夜重置** — 00:00 保存当天数据到历史记录，重置计数器

1. **iCloud** — 通过 NSUbiquitousKeyValueStore 跨设备同步历史 + 设置

1. **日历** — HistoryPanel 读取 drinkHistory，渲染带颜色的月网格
